...
def register_plugin(qt_main_window = None):
    ...
    if qt_main_window is not None and not CONF.get('survey', 'bareme_only') and CONF.get('inequality', 'enable'):
        _ = get_translation('openfisca_qt')
        qt_main_window.set_splash(_("Loading inequality widget ..."))
        widget = widgets.InequalityWidget(qt_main_window)
